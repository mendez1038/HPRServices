...
public class ContenidoServiceTest {
	...
	public void testPrecioDescontado() {
		...
		try {
			...
			int[] all = { 1, 2, 3, 4, 5,6,7,8,9,101,102,103,201 };
			
			for (int i = 0; i < all.length; i++) {
				Contenido c = new Contenido();
				Double precioDescontado = servicio.sacarPrecioDescontado(all[i]);
				c = servicio.findById(all[i], "es");
		...
		try {
			...
			do {
				...
				if (contenidos.getPage().size() > 0) {
					logger.info(
							"Page [" + startIndex + " - " + (startIndex + contenidos.getPage().size() - 1) + "] : ");
